...
QtObject {
  ...
  function request(path, callback) {
    ...
    var url = Api.BASE_URL + String(path || "")
    ...
    var entry = { url: url, key: apiKey, generation: token, done: false, superseded: false,
                  callback: callback, complete: null, abort: null }
  ...
  function startNext() {
    ...
    worker.command = ["curl", "-sS", "--proto", "=https", "--max-filesize",
  ...
  property Process worker: Process {
    ...
    onExited: function(exitCode) {
      ...
      if (entry && !entry.done) entry.complete(root.parseResult(text, exitCode))
      root.startNext()
    }
  }
}
